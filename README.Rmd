...
```{r example_declare_study, warning = FALSE}
study1 <- 
  study_population +
  study_sample_rds + study_sample_pps + study_sample_tls +
  study_estimands +
...
get_meta_estimands <- function(data) {
  ...
  data.frame(inquiry = c(paste0("prevalence_", 1:N)),
             estimand = c(data[,1]),
...
get_meta_estimators = function(data) {
  ...
  data.frame(estimator = c(paste0("prev_", 1:N)),
             estimate = c(apply(fit$alpha, 2, mean)),
             sd =   c(apply(fit$alpha, 2, sd)),
             inquiry = c(paste0("hidden_prev", 1:N)),
             big_Rhat = big_Rhat
             )
