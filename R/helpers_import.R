  ...
  .out <-
    sapply(
      ...
      FUN = function(study) {
        ...
        if (.row$pps == 1) {
          if (!is.na(.row$pps_strata)) {
            .pop$add_groups <-
              c(
                .pop$add_groups,
                strata_for_pps =
                  paste0(
                    "sample(rep(1:", .row$pps_strata ,
                    ", each = ceiling(.N/", .row$pps_strata, ")), size = .N)")
              )
          }
          ...
          if (!is.na(.row$pps_cluster)) {
            ...
            if (is.na(.row$pps_strata))
              stop(paste0("Specify \"strata\" to be 1 for study ", study))
            ...
            .pop$add_groups <-
              c(
                ...
                cluster_for_pps =
                  paste0(
                    "ave(1:.N, strata_for_pps, FUN = function(x) sample(rep(1:",
                    .row$pps_cluster,
                    ", each = ceiling(length(x) / ",
                    .row$pps_cluster,
                    ")), size = length(x)))")
              )
          }
      ...
      FUN = function(study) {
        ...
        .estimators <-
          .estimators[sapply(strsplit(names(.estimators), "_"),
                             function(x) all(x %in% names(append_list[[study]][["samples"]])))]
