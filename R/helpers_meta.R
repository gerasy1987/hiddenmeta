...
get_multi_samples <- function(data, samples_args) {
  ...
  for (st in names(samples_args)) {
    for (samp in names(samples_args[[st]])) {
      data$population[[which(data$study == st)]] <-
        apply_args(study = st,
                   df = data$population[[which(data$study == st)]],
                   args = samples_args[[st]][[samp]])
    }
  }
...
get_multi_estimates <- function(data, estimators_args) {
  ...
  .est_out <-
    data.frame(estimator_label = character(0),
               estimate = numeric(0),
               se = numeric(0),
               inquiry_label = character(0))

  for (st in names(estimators_args)) {
    for (samp in names(estimators_args[[st]])) {
      for (est in names(estimators_args[[st]][[samp]])) {
