...
#' @param assignment_variable character string that is used as prefix for all variables generated by RDS sampling (sample identifier, recruiter ID, wave, time of show-up) in each population
...
get_hpop_rds <- function(data, assignment_variable = "rds", rds_args) {
  ...
  pbapply::pbmapply(
    ...
    FUN = function(pop, samp) {
      ...
      do.call(what = sample_rds,
              args = c(list(data = pop, assignment_variable = assignment_variable),
                       samp[names(samp) %in% formalArgs(sample_rds)]))
