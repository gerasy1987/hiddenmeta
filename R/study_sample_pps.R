...
#' @param weighs_type character string giving the type of weights to compute. Can be one of "absolute" or "relative"
#'
...
sample_pps <-
  function(data, sampling_variable = "pps", drop_nonsampled = FALSE,
    ...
    temp_data <- data <- dplyr::mutate(data, temp_id = 1:n())
    ...
    if (!is.null(cluster)) {
      ...
      temp_data %<>%
        ...
        dplyr::bind_rows(dplyr::filter(temp_data, frame != 1),
                         .) %>%
        dplyr::mutate(across(c(sampled_cluster, sampled), ~ ifelse(is.na(.), 0, 1))) %>%
        dplyr::rename_with(.cols = c(frame, cluster_id, cluster_prop, sampled_cluster, weight),
                           ~ paste0(sampling_variable, "_", gsub("^sampled$", "", x = .))) %>%
        dplyr::rename("{ sampling_variable }" := sampled)
    }
    ...
    if (drop_nonsampled) {
      data %<>% dplyr::filter(dplyr::across(all_of(sampling_variable), ~ . == 1))
    }
