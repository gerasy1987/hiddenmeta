...
study_designs <-
   ...
   read_samples(study_df, append_list = _) |>
   read_inquiries(study_df, append_list = _) |>
   read_estimators(study_df, append_list = _)
```
...
multi_simulations = simulate_design(multi_study, sims = 50, future.seed = 871223)
...
```{r required_data, eval=FALSE, echo=FALSE}
...
data <- 
   ...
   dplyr::filter(study_estimator != "ltsmulti_mse") |> 
...
data |> 
   ggplot(aes(x = study_estimator, y = rel_bias, color = study)) +
   geom_boxplot(notch = TRUE, notchwidth = .8, outlier.shape = NA, color = "black") +
   geom_jitter(size = .7, alpha = .3, width = .2, height = 0) +
   # facet_wrap(~ study, scales = "free_y", ncol = 2) +
   coord_flip() +
   theme_minimal() +
   labs(title = "Relative bias of sampling-estimator strategies",
