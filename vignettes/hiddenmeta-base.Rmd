...
#      vertex.size = 2, vertex.dist = 4, vertex.label = NA, edge.width = .2,
#      edge.arrow.size = .1, edge.curved = .2)
# 
# legend(x = -1, y = -1.2,
    ...
    pop = 
      list(
        ...
        add_groups = 
         ...
         "purrr::map_df(hidden, ~ sapply( `names<-`(c(.2, .1, .3, .2, .4, .1, .3, .2), paste0('loc_', 1:8)), function(add) rbinom(length(.x), 1, 0.1 + .x * add)))",
         known_2 = 0.1, known_3 = 0.2)
      ),
    sample = 
      list(
