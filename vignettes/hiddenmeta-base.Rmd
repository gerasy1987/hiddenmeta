      ...
      list(
        ...
        link_trace =
          list(link_trace = list(handler = get_study_est_linktrace,
                                   total = 300,
                                   strata = "hidden",
                                   gibbs_params = list(n_samples = 5L, chain_samples = 250L, chain_burnin = 50L),
...
GGally::ggnet2(g, palette = "Set1", mode = "fruchtermanreingold",
...
study_sample_link_trace <- 
  eval(as.call(c(list(declare_sampling), study_1$sample$link_trace)))

set.seed(872312)
draw_data(study_population +
            study_sample_rds + study_sample_pps + study_sample_tls + study_sample_link_trace)[sample(1:.N)]%>% 
  DT::datatable(options = list(scrollX = TRUE, pageLength = 15))
...
est_link_trace <- 
  eval(as.call(c(list(declare_estimator), study_1$estimators$link_trace$link_trace)))

set.seed(872312)
draw_estimates(study_population +
                 study_sample_rds + study_sample_pps + study_sample_tls + study_sample_link_trace + 
                 study_estimands +
                 est_ss + est_sspse +
                 est_chords +
                 est_multi +
...
study1 <- 
  ...
  est_nsum_pps + est_ht_pps +
  est_nsum_tls + est_ht_tls + est_recap_tls +
  est_recap_rds_pps + est_recap_rds_tls + est_link_trace
