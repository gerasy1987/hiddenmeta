...
estimator_ht_pps <- do.call(what = declare_estimator, 
                            args = study_1$estimators$pps$ht)
estimator_ht_tls <- do.call(what = declare_estimator, 
                            args = study_1$estimators$tls$ht)

estimator_nsum_tls <- do.call(what = declare_estimator,
                              args = study_1$estimators$tls$nsum)
estimator_nsum_pps <- do.call(what = declare_estimator, 
...
draw_estimates(study_population +
                 study_sample_rds + study_sample_pps + study_sample_tls +
                 study_estimands +
                 estimator_sspse + estimator_chords + 
                 estimator_ht_tls + estimator_ht_pps +
                 estimator_nsum_tls + estimator_nsum_pps)
```
...
```{r meta_study, eval=FALSE, warning = FALSE}
