...
```{r multi_study_diagnose, eval=TRUE}
...
meta_population <- 
  declare_population(multi_design = multi_study, n_sim = 2, parallel = TRUE, 
...
meta_simulations <- simulate_design(meta_study, sims = 2)

diagnose_design(simulations_df = meta_simulations, 
                diagnosands = study_diagnosands) %>% 
  reshape_diagnosis(digits = 2) %>%
  dplyr::select(-'Design Label') %>%
