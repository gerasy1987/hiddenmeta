...
data <- draw_estimates(multi_design)

data %>% 
  dplyr::mutate(
    study = stringr::str_extract(data$inquiry_label, "^study\\_(\\d+)"),
    estimand = stringr::str_remove(pattern = paste0(study, "_"), string = inquiry_label),
    estimator_label = strsplit(estimator_label, "_"),
    inquiry_label = strsplit(inquiry_label, "_"),
    sample = purrr::map2_chr(estimator_label, inquiry_label, 
                             ~ setdiff(.x, .y)[1]),
    estimator = purrr::map2_chr(estimator_label, inquiry_label, 
                                ~ setdiff(.x, .y)[2])
  )
...
set.seed(19872312)
multi_design$multi_sampling(multi_design$multi_population()) %>% 
  kable()
