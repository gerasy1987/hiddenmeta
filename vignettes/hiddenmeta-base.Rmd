...
study1 <- 
  ...
  study_sample_lts + study_sample_ltsmulti +
...
g <-
  study1_data[
    ...
    , c("ltsall_from", "name") :=
      list(plyr::mapvalues(ltsall_from,
...
pal <- 
  grDevices::palette.colors(n = length(unique(igraph::V(g)$ltsall_wave)), 
                            palette = "Dark2",
                            recycle = TRUE) |> 
...
GGally::ggnet2(g, mode = "fruchtermanreingold",
               edge.alpha = .5, edge.size = .2, 
               node.alpha = .9, node.size = 2.5, 
               node.color = "ltsall_wave",
               palette = pal,
               color.legend = "Link-Tracing Wave",
               max_size = 4, shape = 16,
               legend.size = 12, legend.position = "bottom", 
               arrow.gap = 0.01, arrow.size = 6)
...
study1_simulations <- future.apply::future_lapply(
  X = ids,
  FUN = function(x) {
  ...
  future.globals    = list(study1 = study1),
...
study1_simulations <- 
  study1_simulations[sapply(study1_simulations, function(x) class(x) == "data.frame")] |>
  dplyr::bind_rows()
...
meta_simulations <- future.apply::future_lapply(
  X = ids,
  FUN = function(x) {
    # Run one simulation and tag it
    out <- try(
...
meta_simulations <- 
  multi_simulations[sapply(meta_simulations, function(x) class(x) == "data.frame")] |>
  dplyr::bind_rows()
