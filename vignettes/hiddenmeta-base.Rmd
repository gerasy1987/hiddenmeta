...
pal <-
  grDevices::palette.colors(
    n = length(unique(igraph::V(g)$type)),
    palette = "Dark2",
    recycle = TRUE
  ) |>
...
study_sample_pps <- do.call(declare_sampling, study_1$sample$pps)

```

```{r example_tls}

study_sample_tls <- do.call(declare_sampling, study_1$sample$tls)

```

```{r example_lts}

study_sample_lts <- do.call(declare_sampling, study_1$sample$lts)

study_sample_ltsmulti <- do.call(declare_sampling, study_1$sample$lts2)
...
est_multi <- do.call(declare_estimator, study_1$estimators$rds$multiplier)
...
est_ht_tls <- do.call(declare_estimator, study_1$estimators$tls$ht)
est_nsum_tls <- do.call(declare_estimator, study_1$estimators$tls$nsum)
...
est_recap_rds_pps <- do.call(declare_estimator, study_1$estimators$all$recap1)
est_recap_rds_tls <- do.call(declare_estimator, study_1$estimators$all$recap2)
est_mse_rds_pps <- do.call(declare_estimator, study_1$estimators$all$mse1)
est_mse_rds_tls <- do.call(declare_estimator, study_1$estimators$all$mse2)

est_link <- do.call(declare_estimator, study_1$estimators$lts$link)
...
study1 <-
  ...
  est_nsum_tls +
  est_ht_tls +
  est_recap_tls + #est_mse_tls +
  est_recap_rds_pps +
  est_recap_rds_tls + #est_mse_rds_pps + est_mse_rds_tls +
  est_link
...
g <-
  igraph::graph_from_data_frame(
    g[!is.na(name) & !is.na(ltsall_from), .(ltsall_from, name)],
    vertices = g[!is.na(name), .(name, ltsall_wave, ltsall_t)],
    directed = TRUE
  )
...
#       igraph::set_vertex_attr("name", value = name) %>%
#       igraph::set_vertex_attr("type", value = type) %>%
#       igraph::set_vertex_attr("ltsall", value = ltsall) %>%
#       igraph::set_vertex_attr("ltsall_wave", value = ltsall_wave)
#   }
#
# g <- igraph::subgraph(g, which(igraph::V(g)$ltsall == 1))

pal <-
  grDevices::palette.colors(
    n = length(unique(igraph::V(g)$ltsall_wave)),
    palette = "Dark2",
    recycle = TRUE
  ) |>
  `names<-`(unique(igraph::V(g)$ltsall_wave)) |>
...
GGally::ggnet2(
  ...
  node.alpha = .9,
  node.size = 2.5,
  node.color = "ltsall_wave",
  palette = pal,
  color.legend = "Link-Tracing Wave",
  max_size = 4,
  shape = 16,
  legend.size = 12,
  legend.position = "bottom",
...
future::plan(future::multicore, workers = parallel::detectCores() - 1)

study1_simulations <-
  DeclareDesign::simulate_design(study1, sims = 100, future.seed = 20251113)
...
diagnose_design(
  simulations_df = study1_simulations,
...
study_3 <-
  list(
    pop = list(
      ...
      # for service multiplier
      add_groups = list(
        service_use = "rbinom(.N, 1, 0.25 + hidden * 0.05)",
...
```{r multi_study, warning=FALSE}

multi_population <-
  declare_population(
    handler = get_multi_populations,
    study_designs = list(study_1 = study_1, study_3 = study_3)
  )
...
multi_simulations <-
  DeclareDesign::simulate_design(
    multi_study,
    sims = 100,
    future.seed = 20251113
  )
