...
study_1 <- 
  list(
    ...
    estimators = 
      list(
        sspse = list(handler = get_study_est_sspse, label = "sspse"),
        ht = list(handler = get_study_est_ht, label = "ht"),
        chords = list(type = "integrated",
                      handler = get_study_est_chords, 
                      label = "chords"),
        nsum = list(handler = get_study_est_nsum, label = "nsum")
      )
  )
...
```{r example_estimands}
study_estimands <- declare_estimand(handler = get_study_estimands)
...
estimator_sspse <- do.call(what = declare_estimator, args = study_1$estimators$sspse)
estimator_ht <- do.call(what = declare_estimator, args = study_1$estimators$ht)
estimator_chords <- do.call(what = declare_estimator, args = study_1$estimators$chords)
estimator_nsum <- do.call(what = declare_estimator, args = study_1$estimators$nsum)
...
estimator_ht <- do.call(what = declare_estimator, args = study_2$estimators$ht)
estimator_chords <- do.call(what = declare_estimator, args = study_2$estimators$chords)
estimator_nsum <- do.call(what = declare_estimator, args = study_2$estimators$nsum)

set.seed(19872312)
draw_estimates(study_population +
                 study_sample_rds + study_sample_pps +
                 study_estimands +
                 estimator_ht + estimator_chords + estimator_nsum)
...
study_populations <- 
  declare_population(handler = get_study_populations, 
                     pops_args = pops_args)

set.seed(19872312)
( example_pops <- study_populations() )

study_samples <- 
  declare_sampling(handler = get_study_samples, samples_args) 

study_estimands <- 
  declare_estimand(handler = get_study_estimands, study_estimands) 

study_estimators <- 
  declare_estimator(handler = get_study_estimators, study_estimators) 

meta_switch <- 
  declare_step(prep_study_estimators, get_study_estimators)
