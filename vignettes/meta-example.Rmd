...
hidden_prior <- list(mean = 1000, se = 10)
control_params <- list(
...
} else if (!(nrow(.alpha_prior) %in% c(1,.N))) {
...
.fit <- 
  rstan::sampling(object = .stan_model, data = .stan_data,
                  chains = 8, iter = 16000, warmup = 8000, thin = 10)

## does not converge unfortunately
.fit
