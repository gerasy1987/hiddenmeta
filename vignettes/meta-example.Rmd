...
sim_meta <- function(
  ...
  result <- tidyr::expand_grid(possible_samp_est, estimands) |>
    dplyr::mutate(
  ...
  if (pop) {
    result |> 
      dplyr::select(sim_ID, study, inquiry, sample, estimator, estimand, estimate, se, everything()) |> 
      as.data.frame()
  } else {
    result |> 
      get_meta_sample(data = _) |> 
      dplyr::filter(meta == 1)
  }
}
...
meta_simulations = simulate_design(meta_study, sims = 200, future.seed = 872312)
...
study_designs_filtered <- study_designs_base |> 
  dplyr::group_by(sim_ID, sample, estimator) |> 
  dplyr::filter(dplyr::n() >= 3) |> 
  dplyr::ungroup()

meta_population <- declare_population(study_designs_filtered)
...
meta_simulations = simulate_design(meta_study, sims = 100, future.seed = 872312)
...
for (i in seq_along(study_map)) {
  ...
  meta_simulations <- future.apply::future_lapply(
    ...
    future.globals    = list(meta_study = meta_study, 
                             study_designs_filtered = study_designs_filtered),
...
leave1out_sims <- 
  lapply(study_map, function(x) {
    ...
    result <- readRDS(system.file("extdata", paste0("meta_sim_real_excl_", x, ".rds"), 
                        package = "hiddenmeta")) |> 
      diagnose_design(simulations_df = _,
                      diagnosands = study_diagnosands)
    
      result$diagnosands_df |> 
      dplyr::filter(!is.na(mean_estimate)) |> 
      dplyr::mutate(study_excluded = x) |>
      dplyr::mutate(!!sym(x) := rmse/mean_estimand) |> 
      dplyr::select(c(inquiry, all_of(x)))
    
  }) 

diagnosands_result <- readRDS(system.file("extdata", "meta_sim_real_original.rds", package = "hiddenmeta")) |> 
  diagnose_design(simulations_df = _,
                  diagnosands = study_diagnosands)

table_leave1out <- diagnosands_result$diagnosands_df |> 
  dplyr::filter(!is.na(mean_estimate)) |> 
  dplyr::mutate(all = abs(rmse/mean_estimand)) |> 
  dplyr::select(inquiry, all) |>
  (\(init_df) purrr::reduce(leave1out_sims, dplyr::left_join, by = "inquiry", .init = init_df))() |>
