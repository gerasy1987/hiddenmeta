...
```{r declare_meta_base, eval=FALSE, include=FALSE}
...
get_meta_sample_2 <- function(data) {
  data %>% 
    dplyr::mutate(
      meta = as.integer(sim_ID == sample(1, unique(sim_ID)))
    )
}

get_meta_estimates_2 <- function(
  data,
  sampling_variable = "meta",
  which_estimand = "hidden_size",
  benchmark = list(estimator = "ht"),
  stan_handler = get_meta_stan2,
  control_params = list(
  ...
  .benchmark_sample_est <-
    which(.samp_ests$estimator == benchmark$estimator)
  .samp_ests <-
...
meta_population <-
  declare_population(study_designs_base)
...
data <- draw_data(meta_population + meta_sample)
